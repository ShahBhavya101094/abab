...
if (isset($_POST['update'])) {
    ...
    $query = "UPDATE `user_reg` SET `pranat`=?,`vibhag`=?,`dayitwa`=?,`fullname`=?,`gender`=?,`address_info`=?,`mobile`=?,`email`=?,  `dob`=?,district= ?,adhar_number=?  WHERE id = ?";
     $stmt = $mysqli->prepare($query);
     $rc = $stmt->bind_param('ssssssssssss', $pranat, $vibhag, $dayitwa, $fullname, $gender, $address, $mobile,$email,$dob,$district, $adhar_number,$aid );
     $stmt->execute();
